...
from model_matrix import ELiCiT
import torch
...
class Trainer(nn.Module):
    def __init__(self, filename, num_feats, batch_size=524288, qlevel=4, gpus=[0, 1, 2, 3]):
        ...
        self.qlevel = qlevel
        self.load(filename)
        self.model = ELiCiT(dims=(65536, 65536), num_feats=num_feats, qlevel=qlevel)
    ...
    def eval_model(self):
        ...
        self.model.eval()
        vals = self.model.predict([self.test_srcs, self.test_dsts], n_neg=0)
    ...
    def handle_model(self, optimizer=None, save=False):
        ...
        n_neg = (65536 // (2 * n_samples))
        outs = self.model([target_srcs, target_dsts], n_neg=n_neg)
        vals, feats, neg_vals, neg_feats = outs['vals'], outs['feats'], outs['neg_vals'], outs['neg_feats']
    ...
    def train_iteration(self, optimizer, cache_size=0, neg_cache_size=0):
        ...
        for epoch_cnt in range(0, self.train_vals.shape[0], 1):
            self.model.eval()
            with torch.no_grad():
                prev_value = self.model.predict([self.train_srcs[epoch_cnt:(epoch_cnt+1)], self.train_dsts[epoch_cnt:(epoch_cnt+1)]], n_neg=1)
                prev_value_sum += prev_value
                cache_hit = -1
...
if __name__ == '__main__':
    ...
    parser.add_argument("--input-path", default='example_data', type=str, help="target tensor for compression")
