...
if __name__ == '__main__':
    ...
    total_num_steps = num_epochs * num_steps_per_epoch[task]
    fname = f'{task}-{lr}-{wd}-{batch_size}-{num_epochs}-{seed}'
    
    finetuned_checkpoint = f'{save_path}/{fname}/checkpoint-{total_num_steps}'
    ...
    for k, v in tqdm.tqdm(original_state_dict.items()):
        if ('bert.encoder.layer' in k and 'weight' in k) and (len(v.shape) >= 2):
            v = v.to(0)
            w_orig = torch.load(f'{save_path}/fisher_{fname}/{k}.pkt').to(0)
            decomposed_state_dict[k] = get_matrices(k, v, rank, w_orig)
            
    torch.save(decomposed_state_dict, f'{fname}.pkt')